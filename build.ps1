...
# Run user-specific script if necessary.

foreach($fileName in @("before-build.ps1", "startup.ps1"))
{
	$preBuildScript = Join-Path $PSScriptRoot $fileName;
	if (Test-Path $preBuildScript) { &$preBuildScript; }
}
...
if ($Preview) { $hostEnvironmentName = "preview"; }
if ($Production) { $hostEnvironmentName = "production"; }

if ([string]::IsNullOrEmpty($SecretsFilePath)) { $SecretsFilePath = (Join-Path $PSScriptRoot "secrets.json"); }

$currentBranch = try { (git branch --show-current).Trim() } catch { $null }
...
else
{
	...
	Write-Host -ForegroundColor DarkGray "Environment:   $hostEnvironmentName";
